  ...
  // console.log(!user.rating)
    ...
    const handleLogout =async ()=>{
      try {
        ...
        console.log("in appbar",resp.data);
        setUser(null);
      } catch (error) {
        console.log(error);
      }
...
<nav className="bg-white border-gray-200 dark:bg-gray-900">
  ...
  <div className="flex md:order-2">
    ...
    <button data-collapse-toggle="navbar-search" type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 darkF:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-search" aria-expanded="false">
    ...
    <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
      ...
      <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
        ...
        <li>
         ...
         ):(<p className="text-white text-md cursor-pointer" onClick={handleLogout}>Logout</p>)}
        </li>
        <li>
          <Link to="/connect" className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">Connect</Link>
        </li>
      </ul>
    </div>
  </div>
